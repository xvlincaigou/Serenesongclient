  ...
  <view class="container">
    ...
    <view class="section">
      <view class="avatar-wrapper">
        <image class="avatar">{{ friendAvatar }}</image>
      </view>
      <view class="stats">
        <text>创作<br>23</text>
        <text>收藏<br>15</text>
    ...
    <scroll-view v-if="selectedTab === 'dynamic'" class="content" scroll-y="true">
      <view v-for="(post, index) in dynamicPosts" :key="index">
        ...
        <view class="post-header">
          <image class="post-avatar">{{ friendAvatar }}</image>
          <view class="post-info">
            <text class="post-nickname">{{ friendName }}</text>
        ...
        <view class="post-actions">
          <button @click.stop="share(post)">分享 996</button>
          <button @click.stop="comment(post)">评论 2000+</button>
          <button @click.stop="love(post)" :style="{backgroundColor: post.liked ? 'red' : 'white', color: post.liked ? 'white' : 'grey'}">点赞 410</button>
        </view>
      </view>
    </scroll-view>
...
export default {
  ...
  onLoad(options) {
    ...
    this.friendAvatar = options.avatar;
  },
  methods: {
    selectTab(tab) {
	...
	viewPost(post) {
	  uni.navigateTo({
	    url: `/pages2_community/postDetail/postDetail?title=${encodeURIComponent(post.title)}&date=${post.date}&content=${encodeURIComponent(post.content)}&avatar=${encodeURIComponent(this.avatar)}&nickname=${encodeURIComponent(this.friendName)}&category=${encodeURIComponent(post.category)}&detail=${encodeURIComponent(post.detail)}`
	  });
	},
	share(item) {
	  uni.showToast({
	...
	comment(item) {
	  uni.navigateTo({
	    url: `/pages2_community/postDetail/postDetail?title=${encodeURIComponent(item.title)}&date=${item.date}&content=${encodeURIComponent(item.content)}&avatar=${encodeURIComponent(this.avatar)}&nickname=${encodeURIComponent(this.friendName)}&category=${encodeURIComponent(item.category)}&detail=${encodeURIComponent(item.detail)}`
	  });
	},
	love(item) {
...
.content { max-height: 350px; overflow-y: auto; padding: 0px; margin-right: 50px; font-size: 10px;}
