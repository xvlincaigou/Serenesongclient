...
export default {
  ...
  methods: {
	...
	viewPost(post) {
	  uni.navigateTo({
	    url: `/pages5_user/postDetail/postDetail?title=${encodeURIComponent(post.title)}&date=${post.date}&content=${encodeURIComponent(post.content)}&avatar=${encodeURIComponent(this.avatar)}&nickname=${encodeURIComponent(this.friendName)}&category=${encodeURIComponent(post.category)}&detail=${encodeURIComponent(post.detail)}`
	  });
	},
	share(item) {
	  uni.showToast({
	...
	comment(item) {
	  uni.navigateTo({
	    url: `/pages5_user/postDetail/postDetail?title=${encodeURIComponent(item.title)}&date=${item.date}&content=${encodeURIComponent(item.content)}&avatar=${encodeURIComponent(this.avatar)}&nickname=${encodeURIComponent(this.friendName)}&category=${encodeURIComponent(item.category)}&detail=${encodeURIComponent(item.detail)}`
	  });
	},
	love(item) {
