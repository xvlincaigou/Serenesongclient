...
export default {
  ...
  methods: {
    ...
    viewWorkDetail(work) {
        uni.navigateTo({
            url: `/pages/user/favoriteDetail/favoriteDetail?cipai=${encodeURIComponent(work.cipai)}&author=${encodeURIComponent(work.author)}&content=${encodeURIComponent(JSON.stringify(work.content))}&comment=${encodeURIComponent(work.comment)}&collectionID=${this.collectionID}&ciID=${work.ciID}&token=${this.token}`
        });
    },
  }
}
